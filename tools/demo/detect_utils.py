...
def detect_objects_on_multiple_images(model, transforms, cv2_images):
    ...
    for cv2_img in cv2_images:
        ...
        img_list.append(img_input)

    with torch.no_grad():
        predictions = model(img_list)
    
    pred_results = []
    for prediction, cv2_img in zip(predictions, cv2_images):
        ...
        if isinstance(model, SceneParser):
            ...
            pred_results.append({'objects': rt_box_list, 'relations':rt_relation_list})
            continue
        else:
            if 'attr_scores' in prediction.extra_fields:
                attr_scores = prediction.get_field("attr_scores")
